...
export default async function PengaturanPage() {
  const [systemSettingsData, role] = await Promise.all([
    getSystemSettings(),
    getSessionRole(),
  ]);
  ...
  return (
    <PageWrapper
      title="Pengaturan"
      description="Pusat status konfigurasi pendukung sistem. Secret tetap dikelola melalui environment server."
    >
      <div className="space-y-6">
        <IdentitasSistemCard initial={systemSettingsData} isAdmin={role === "admin"} />
        <section className="grid gap-4 lg:grid-cols-3">
          <SummaryCard
            icon={HardDrive}
            label="Storage"
            value={storageProvider}
            status={storageProvider === "local" ? "Development ready" : "Provider eksternal"}
          />
          <SummaryCard
            icon={Cloud}
            label="Cloudinary"
          ...
          <Card className="rounded-[24px]">
            ...
            <CardContent className="space-y-3 text-sm leading-6 text-muted-foreground">
              <div className="rounded-2xl border border-border bg-muted/35 p-4">
                Provider, batas file, dan status integrasi bisa dipantau di sini. Perubahan
                secret seperti database URL, auth secret, API key, dan API secret tetap dilakukan
                melalui file environment atau panel hosting.
              </div>
              <div className="rounded-2xl border border-border bg-muted/35 p-4">
                Jika Cloudinary belum diputuskan, sistem tetap bisa berjalan dengan storage
                lokal selama skenario upload sudah tervalidasi untuk kebutuhan operasional.
              </div>
            </CardContent>
          </Card>
        </section>
