...
import {
  ...
  generateQrSuratKeputusan,
  type SuratKeputusanRow,
} from "@/server/actions/suratKeputusan";
import { SuratKeputusanForm } from "./SuratKeputusanForm";

type FormState =
  | { open: false }
  | { open: true; mode: "create" }
  | { open: true; mode: "edit"; row: SuratKeputusanRow };

export function SuratKeputusanManager({
  initialData,
  pejabatList,
  role,
}: {
  initialData: SuratKeputusanRow[];
  ...
  const [deleteTarget, setDeleteTarget] = useState<SuratKeputusanRow | null>(null);
  ...
  const filteredData = useMemo(() => {
    ...
    return initialData.filter((row) =>
      [row.nomorSK, row.perihal, row.tentang, row.pejabatNama ?? ""]
        .join(" ")
        .toLowerCase()
        .includes(keyword),
    );
  ...
  function handleDeleteConfirm() {
    ...
    startDeleteTransition(async () => {
      const result = await deleteSuratKeputusan({ id: deleteTarget.id });
      if (!result.ok) {
        toast.error(result.error);
        return;
      }

      toast.success(`Surat Keputusan ${deleteTarget.nomorSK} dihapus.`);
      setDeleteTarget(null);
      router.refresh();
    });
  }

  async function handleGenerateQr(row: SuratKeputusanRow) {
    setQrLoadingId(row.id);
    try {
      const result = await generateQrSuratKeputusan({ id: row.id });
      if (!result.ok) {
        toast.error(result.error);
        return;
      }
      toast.success("QR verifikasi Surat Keputusan berhasil dibuat.");
      router.refresh();
    } catch (error) {
      toast.error(
  ...
  function handleExportCsv() {
    exportRowsToCsv(
      initialData.map((row) => ({
        nomor_sk: row.nomorSK,
        perihal: row.perihal,
        tentang: row.tentang,
        tanggal_sk: row.tanggalSK,
  ...
  return (
    <div className="space-y-6">
      <section className="grid gap-4 md:grid-cols-3">
        <SummaryCard label="Total SK" value={String(initialData.length)} hint="Data surat keputusan yang tercatat" />
        ...
        <CardHeader className="border-b border-border">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <CardDescription>
                Kelola nomor SK, metadata keputusan, file dokumen, dan QR verifikasi.
              </CardDescription>
            </div>
            {canManage ? (
          ...
          <div className="pt-2">
            <Input
              ...
              placeholder="Cari nomor SK, perihal, tentang, atau pejabat..."
              className="max-w-md"
            />
          </div>
        </CardHeader>
        <CardContent className="grid gap-4 p-6 md:grid-cols-2">
          {filteredData.length ? (
            filteredData.map((row) => {
              const verificationUrl = `/verifikasi/surat-keputusan/${row.id}`;
              return (
                <Card key={row.id} className="rounded-[24px] border border-border shadow-none">
                  <CardContent className="space-y-4 p-5">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="font-mono text-sm font-semibold text-primary">{row.nomorSK}</p>
                        <p className="mt-2 font-medium text-foreground">{row.perihal}</p>
                        <p className="mt-1 text-sm text-muted-foreground">{formatTanggal(row.tanggalSK)}</p>
                      </div>
                      {canManage ? (
                        <DropdownMenu>
                    ...
                    <div className="space-y-2 text-sm text-muted-foreground">
                      <p>Tentang: {row.tentang}</p>
                      <p>Berlaku: {row.tanggalBerlaku ? formatTanggal(row.tanggalBerlaku) : "-"}</p>
                      <p>Berakhir: {row.tanggalBerakhir ? formatTanggal(row.tanggalBerakhir) : "-"}</p>
                    </div>
            ...
            <div className="md:col-span-2 rounded-3xl border border-dashed border-border bg-muted/20 px-6 py-14 text-center text-sm text-muted-foreground">
              Belum ada Surat Keputusan yang cocok dengan pencarian.
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => !open && !isDeleting && setDeleteTarget(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Hapus Surat Keputusan?</DialogTitle>
            <DialogDescription>
              Dokumen <span className="font-medium text-foreground">{deleteTarget?.nomorSK}</span> akan dihapus dari sistem.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
