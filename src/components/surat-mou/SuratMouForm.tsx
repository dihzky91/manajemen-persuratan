...
const formSchema = z.object({
  ...
  tanggalMOU: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Format tanggal harus YYYY-MM-DD"),
    ...
    startTransition(async () => {
      try {
        ...
        if (selectedFile) {
          ...
          const uploadResult = await uploadSuratMouFile({
          ...
          if (!uploadResult.ok) {
            toast.error("Upload file MOU gagal.");
            return;
          }
        ...
        const result =
          mode === "edit" && initialData
            ? await updateSuratMou({ ...payload, id: initialData.id })
            : await createSuratMou(payload);
        ...
        toast.success(
          mode === "edit" ? "Surat MOU diperbarui." : "Surat MOU berhasil dibuat.",
        );
        router.refresh();
        onOpenChange(false);
      } catch (error) {
        toast.error(
          error instanceof Error ? error.message : "Gagal menyimpan Surat MOU.",
        );
      }
    });
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {mode === "edit" ? "Ubah Surat MOU" : "Tambah Surat MOU"}
          </DialogTitle>
          <DialogDescription>
            Tanggal MOU tetap bersifat manual dan mendukung backdate sesuai kebutuhan administrasi.
          </DialogDescription>
        </DialogHeader>

        <Form {...form}>
          <form id="surat-mou-form" className="space-y-4" onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid gap-4 md:grid-cols-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Nomor MOU</FormLabel>
                    <FormControl>
                      <Input autoFocus placeholder="Mis. 001/MOU/IAI-DKIJKT/IV/2026" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Perihal MOU" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Nama pihak kedua" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Alamat Pihak Kedua</FormLabel>
                  <FormControl>
                    <Textarea rows={2} placeholder="Alamat pihak kedua" {...field} value={field.value ?? ""} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Nilai Kerjasama</FormLabel>
                  <FormControl>
                    <Textarea rows={2} placeholder="Opsional, mis. ruang lingkup atau nilai kerjasama" {...field} value={field.value ?? ""} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
        ...
        <DialogFooter>
          ...
          <Button type="submit" form="surat-mou-form" disabled={isPending}>
            {isPending ? "Menyimpan..." : mode === "edit" ? "Simpan Perubahan" : "Simpan MOU"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
